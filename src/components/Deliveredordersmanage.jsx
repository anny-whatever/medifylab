...
function Deliveredordersmanage() {
  ...
  const handleMoveToAttended = (order) => {
    const docRef = doc(db, "orderHandlers", "orders");
    updateDoc(docRef, {
      acceptedOrders: arrayRemove(order),
      deliveredOrders: arrayUnion(order),
    });
  };
    ...
    <>
      ...
      <div className="flex flex-col w-4/5 p-2 mx-auto rounded-lg shadow-lg border-1 h-fit">
        ...
        {data?.deliveredOrders &&
          [...data.deliveredOrders].reverse().map((item, index) => {
            return (
              <>
                <div
...
export default Deliveredordersmanage;
